    ...
    <View style={ms.sheet}>
      ...
      <View style={ms.fields}>
        <View style={ms.field}>
          ...
          <TextInput ref={nameRef} style={ms.input} value={name} onChangeText={setName} placeholder="e.g. Joe's Pizza" placeholderTextColor={Colors.inkDisabled} returnKeyType="next" />
        </View>
        <View style={ms.field}>
          <Text style={ms.label}>Address (optional)</Text>
          <TextInput style={ms.input} value={address} onChangeText={setAddress} placeholder="e.g. 7 Carmine St, New York" placeholderTextColor={Colors.inkDisabled} returnKeyType="done" onSubmitEditing={handleConfirm} />
        </View>
      </View>
...
const ms = StyleSheet.create({
  sheet: { backgroundColor: Colors.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 32, borderWidth: 1, borderColor: Colors.border },
  ...
  field: { gap: 6 },
  label: { fontSize: 12, fontWeight: "700", color: Colors.inkSecondary, textTransform: "uppercase", letterSpacing: 0.5 },
  input: { height: 50, borderWidth: 1.5, borderColor: Colors.border, borderRadius: 12, paddingHorizontal: 14, fontSize: 15, color: Colors.ink, backgroundColor: Colors.surface },
  actions: { flexDirection: "row", gap: 10 },
  cancelBtn: { flex: 1, height: 48, borderRadius: 12, borderWidth: 1.5, borderColor: Colors.border, alignItems: "center", justifyContent: "center" },
  cancelText: { fontSize: 15, fontWeight: "600", color: Colors.inkSecondary },
  confirmBtn: { flex: 2, height: 48, borderRadius: 12, backgroundColor: Colors.accent, alignItems: "center", justifyContent: "center" },
      ...
      <View style={s.container}>
        ...
        <View style={s.searchRow}>
          <View style={s.searchBar}>
            ...
            <TextInput ref={searchRef} style={s.searchInput} value={query} onChangeText={setQuery} placeholder="Search restaurants…" placeholderTextColor={Colors.inkDisabled} returnKeyType="search" autoCorrect={false} clearButtonMode="while-editing" />
          ...
          <FlatList
            ...
            renderItem={({ item: pred }) => (
              <Pressable style={({ pressed }) => [s.row, pressed && s.rowPressed]} onPress={() => handleSelectPrediction(pred)}>
                <View style={s.rowIcon}><Ionicons name="location-outline" size={18} color={Colors.accent} /></View>
              ...
              <>
                ...
                {recents.map((r) => (
                  <Pressable key={r.id} style={({ pressed }) => [s.row, pressed && s.rowPressed]} onPress={() => handleSelectRecent(r)}>
                    <View style={s.rowIcon}><Ionicons name="time-outline" size={18} color={Colors.inkSecondary} /></View>
                    <View style={s.rowBody}>
...
const s = StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 14, backgroundColor: Colors.card, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.border },
  closeBtn: { width: 38, height: 38, borderRadius: 10, backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.border, alignItems: "center", justifyContent: "center" },
  headerTitle: { fontSize: 17, fontWeight: "700", color: Colors.ink },
  searchRow: { padding: 14, backgroundColor: Colors.card, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.border },
  searchBar: { flexDirection: "row", alignItems: "center", height: 46, backgroundColor: Colors.surface, borderRadius: 13, paddingHorizontal: 12, borderWidth: 1.5, borderColor: Colors.border, gap: 8 },
  searchInput: { flex: 1, fontSize: 15, color: Colors.ink },
  list: { paddingHorizontal: 16, paddingTop: 8, paddingBottom: 24 },
  sectionLabel: { fontSize: 11, fontWeight: "700", color: Colors.inkSecondary, textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 8, marginTop: 4 },
  row: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 13, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.border },
  rowPressed: { opacity: 0.6 },
  rowIcon: { width: 36, height: 36, borderRadius: 10, backgroundColor: Colors.accentLight, alignItems: "center", justifyContent: "center" },
  rowBody: { flex: 1 },
  rowName: { fontSize: 15, fontWeight: "600", color: Colors.ink },
  rowAddr: { fontSize: 12, color: Colors.inkSecondary, marginTop: 2 },
  noRes: { paddingVertical: 24, alignItems: "center" },
  noResText: { fontSize: 14, color: Colors.inkSecondary },
  ...
  manualText: { fontSize: 14, fontWeight: "600", color: Colors.accent },
