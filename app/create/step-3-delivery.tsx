    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.flex}>
        ...
        <View style={styles.content}>
          {error && <View style={styles.errorBanner}><Ionicons name="alert-circle" size={15} color={Colors.danger} /><Text style={styles.errorText}>{error}</Text></View>}
          <Text style={styles.label}>Where should we deliver?</Text>
          <TextInput style={styles.input} value={address} onChangeText={setAddress} placeholder="Enter your delivery address…" placeholderTextColor={Colors.inkDisabled} multiline numberOfLines={2} />
          <Pressable style={styles.detectBtn} onPress={handleDetect} disabled={isDetecting}>
            {isDetecting ? <ActivityIndicator size="small" color={Colors.accent} /> : <Ionicons name="locate" size={18} color={Colors.accent} />}
        ...
        <View style={styles.footer}>
          <Pressable style={[styles.nextBtn, !address.trim() && styles.nextBtnDisabled]} onPress={handleNext} disabled={!address.trim()}>
            ...
            {!!address.trim() && <Ionicons name="arrow-forward" size={18} color={Colors.white} />}
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1.5, borderColor: Colors.border, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, color: Colors.ink, backgroundColor: Colors.card, minHeight: 64, textAlignVertical: "top" },
  detectBtn: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 12, paddingHorizontal: 14, borderRadius: 12, borderWidth: 1.5, borderColor: Colors.accentLight, backgroundColor: Colors.accentLight },
  detectText: { color: Colors.accent, fontWeight: "600", fontSize: 14 },
